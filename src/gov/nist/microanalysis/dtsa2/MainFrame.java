...
public class MainFrame extends JFrame {
   ...
   private final JButton jButton_SpecDown = createButton("down1_sm.png", "Move the selected spectra down in the list.");
    ...
    * @param delimInside
    *           whether or not delim should be included in chunk before length
    *           reaches width.
   ...
   /**
    * A method for invoking a Runnable object on the Event Dispatch thread. If
    * the current thread is the Event Dispatch thread then the Runnable is just
    * run. Otherwise the SwingUtilities method invokeAndWait is called.
    *
    * @param as
    */
   ...
   /**
    * Call once after all the windows have been created to write items to the
    * HTML report.
    */
         ...
         public void actionPerformed(ActionEvent e) {
            if (!mInside) {
               ...
               try {
                  ...
                  if (DataManager.isSet(event, DataManager.ADD_SPECTRUM) || DataManager.isSet(event, DataManager.REMOVE_SPECTRUM)
   ...
   private void updateDefaultDetector() {
      ...
      AppPreferences.getInstance().setDefaultDetector(sel instanceof DetectorProperties ? (DetectorProperties) sel : null);
   ...
   private void initialize() throws Exception {
      ...
      /*
       * jMenu_PlugIn.setMnemonic(KeyEvent.VK_P);
       * jMenu_PlugIn.setText("Plug-Ins"); addPlugInMenuItem(jMenuItem_Install);
       */

      jMenuItem_SaveAs.setMnemonic(KeyEvent.VK_A);
      jMenuItem_SaveAs.setText("Save As");
      jMenuItem_SaveAs.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
      ...
      jMenuItem_PrintSpectra.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_P, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
      ...
      jMenuItem_Open.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
      ...
      jMenuFile_Exit.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx()));
      ...
      final PanelBuilder pb1 = new PanelBuilder(new FormLayout("175dlu", "pref, 2dlu, fill:pref:grow"), jPanel_SpecList);
      ...
      {
         PanelBuilder bbb = new PanelBuilder(new FormLayout("fill:pref:grow, pref, 10dlu, pref, 10dlu, pref, 10dlu,pref, fill:pref:grow", "pref"));
         ...
         copy.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
               ...
               if (jTable_SpecComposition.getModel() instanceof ParticleSignatureTableModel) {
                  final ParticleSignatureTableModel model = (ParticleSignatureTableModel) jTable_SpecComposition.getModel();
                  final StringSelection ss = new StringSelection(model.toString());
                  getToolkit().getSystemClipboard().setContents(ss, null);
               }
      ...
      addUtilityTab("Report", tab, "An HTML summary of this DTSA-II session");
      {
         ...
         jTabbedPane_Utility.addTab("Script", ii, jPanel_Command, "A command line for scripting DTSA-II in Python");
      }
      /*
       ...
       * jPanel_MapViewer.setTileFactory(tileFactory); // Use 8 threads in
       * parallel to load the tiles tileFactory.setThreadPoolSize(8);
       * MouseInputListener mia = new PanMouseInputListener(jPanel_MapViewer);
       * jPanel_MapViewer.addMouseListener(mia);
       * jPanel_MapViewer.addMouseMotionListener(mia);
       * jPanel_MapViewer.addMouseListener(new
       * CenterMapListener(jPanel_MapViewer));
       * jPanel_MapViewer.addMouseWheelListener(new
       * ZoomMouseWheelListenerCursor(jPanel_MapViewer));
       * jPanel_MapViewer.addKeyListener(new PanKeyListener(jPanel_MapViewer));
       * final ImageIcon ii = new
       * ImageIcon(MainFrame.class.getResource("osm.png"));
       * jTabbedPane_Utility.addTab("Map", ii, new
       * JScrollPane(jPanel_MapViewer), "An OpenStreetMaps-based map view.");
       * jPanel_MapViewer.setZoom(4); jPanel_MapViewer.setCenterPosition(new
       * GeoPosition(39.13260, -77.21694)); }
       */
            ...
            try {
               ...
               if (dtde.getTransferable().isDataFlavorSupported(flavor)) {
                  ...
                  if (data instanceof List<?>) {
                     ...
                     if (failed.size() > 0) {
                        ErrorDialog.createErrorMessage(MainFrame.this, "Drag-and-Drop", "Unable to open " + failed.size() + " dropped files.",
                              failed.toString());
                     }
                  }
               }
   ...
   public static File selectFileDestination(String fn, Component parent) {
      ...
      jfc.addChoosableFileFilter(new SimpleFileFilter(new String[]{"tia.msa"}, "TIA Compatible EMSA/MAS Standard"));
      jfc.addChoosableFileFilter(new SimpleFileFilter(new String[]{"tif", "tiff"}, "ASPEX-style TIFF Spectrum"));
      final SimpleFileFilter emsa = new SimpleFileFilter(new String[]{"msa", "emsa", "txt"}, "EMSA/MAS Standard File");
   ...
   public void toolsEditStandards() {
      final StandardDatabaseEditor sdbe = new StandardDatabaseEditor(MainFrame.this, DTSA2.getStandardsDatabase(), DTSA2.getSession());
   ...
   /*
    * private class PluginAction extends AbstractAction { private static final
    * long serialVersionUID = -6928508033213127087L; private final PyFunction
    * mFunction; public PluginAction(String name, PyFunction func) {
    * super(name); mFunction = func; }
    *
         ...
         public void run() {
            ...
            if (option == JFileChooser.APPROVE_OPTION)
               try {
                  ...
                  final String title = "Report" + (m.find() ? " " + m.group(1) : "") + ": " + f.getParentFile().getName();
   ...
   private void importSpectra(final ISpectrumData[] specs, boolean scrollTo) {
      if (specs != null) {
         ...
         for (int j = 0; j < specs.length; ++j) {
                     ...
                     if (!assign) {
                        final int opt = JOptionPane.showConfirmDialog(MainFrame.this,
                              "<html>The calibration of <i>" + sd.toString() + "</i><br>"
                                    + "does not seem to be similar to the default detector.<br><br>" + "Apply the default detector none the less?",
                              "Spectrum open", JOptionPane.YES_NO_CANCEL_OPTION);
                        if (opt == JOptionPane.CANCEL_OPTION) {
                           if ((j == 0) && (specs.length > 1))
                              if (JOptionPane.showConfirmDialog(MainFrame.this, "Cancel all " + Integer.toString(specs.length) + " spectra?",
                        ...
                        if (opt == JOptionPane.NO_OPTION)
                           if ((j == 0) && (specs.length > 1))
                              dontApplyToAll = JOptionPane.showConfirmDialog(MainFrame.this,
                                    "Don't apply the default detector to all " + Integer.toString(specs.length) + " spectra?", "File Open",
                                    JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
                        assign = (opt == JOptionPane.YES_OPTION);
                     }
                     if (assign)
                        sd = SpectrumUtils.applyEDSDetector(EDSDetector.createDetector(det, (EDSCalibration) dc), sd);
                  }
               }
            }
            ...
            addSpectrum(sd, true);
            list.add(sd.getProperties().asURL(sd).toString());
         }
         if (list.size() > 0) {
            StringBuilder sb = new StringBuilder();
            if (list.size() == 1)
               sb.append("<p>Opening " + list.get(0) + " from disk.");
            else {
      ...
      final Runnable r = new Runnable() {
         ...
         public void run() {
            try {
                  ...
                  if (msd.shouldSave() && (!msd.isCancelled())) {
                     ...
                     if (res != null) {
                        ...
                        for (final ISpectrumData spec : spectra)
                           cpnas.add(SpectrumUtils.totalCounts(spec, true) / SpectrumUtils.getDose(spec.getProperties()));
                        if (spectra.size() > 1) {
                           hl.append("<p><table>\n");
                           hl.append(
                           ...
                           for (final ISpectrumData spec : spectra) {
                              ...
                              hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.BeamEnergy, Double.NaN)) + "</td>");
                              hl.append("<td>" + hu3.format(SpectrumUtils.getAverageFaradayCurrent(sp, Double.NaN)) + "</td>");
                              hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN)) + "</td>");
                              final UncertainValue2 uv2 = UncertainValue2.divide(
                                    UncertainValue2.createGaussian(SpectrumUtils.totalCounts(spec, true), spec.toString()),
                              ...
                              "<tr><th>Spectrum</th><th>Beam Energy<br>(keV)</th><th>Probe Current<br>(nA)</th><th>Live Time<br>(s)</th><th>Counts/(nA\u00B7S)</th></tr>\n");
                        {
                           ...
                           hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN)) + "</td>");
                           final UncertainValue2 uv2 = UncertainValue2.divide(
                                 UncertainValue2.createGaussian(SpectrumUtils.totalCounts(res, true), res.toString()), SpectrumUtils.getDose(sp));
                           hl.append("<td>" + uv2.format(hu) + "</td>");
                           hl.append("</tr>\n");
                        }
                        hl.append("</table></p>");
               ...
               JOptionPane.showMessageDialog(MainFrame.this, e.getMessage(), "Error creating a standard", JOptionPane.INFORMATION_MESSAGE);
            }
         }
      };
      invokeCarefully(r);
            ...
            if (wiz.showWizard() == JWizardDialog.FINISHED) {
               ...
               if (resSpectra.size() > 0) {
                  ...
                  for (final ISpectrumData spec : resSpectra) {
                     if ((spec instanceof DerivedSpectrum) && (((DerivedSpectrum) spec).getBaseSpectrum() != null))
   ...
   private void displaySpectrumProperties() {
      final Runnable r = new Runnable() {
         ...
         public void run() {
            ...
            Composition comp = tm.getProperties().getCompositionWithDefault(SpectrumProperties.MicroanalyticalComposition, null);
            if (comp != null)
   ...
   void jButtonClear_actionPerformed(ActionEvent e) {
      List<ISpectrumData> specs = getSelectedSpectra();
      if (specs.size() > 0) {
         final int lineLen = 80;
         StringBuilder sb = new StringBuilder();
         sb.append("<h3>Clearing spectra from memory...</h3>\n<p>&nbsp;&nbsp;&nbsp;&nbsp;");
         mDataManager.removeSpectra(specs);
         int l = 0;
         ISpectrumData last = specs.get(specs.size() - 1);
         for (final ISpectrumData sd : specs) {
            l += sd.toString().length();
            if (l > lineLen) {
               sb.append("<br/>&nbsp;&nbsp;&nbsp;&nbsp;");
               l = sd.toString().length();
            }
   ...
   public void fileSaveAs() {
         ...
         if (f != null)
            try {
               if (f.getName().endsWith(".csv")) {
                  ...
                  list.add("<i>" + sd.toString() + "</i> written to <i>" + f.toString() + "</i> as a comma-separated values formatted file.");
               } else if (f.getName().endsWith(".tia.msa")) {
                  exportSpectrumAsTiaEMSA(sd, f.getCanonicalPath());
                  list.add("<i>" + sd.toString() + "</i> written to <i>" + f.toString() + "</i> as a TIA compatible EMSA/MSA 1.0 formatted file.");
               } else if (f.getName().endsWith(".tif")) {
                  exportSpectrumAsTIFF(sd, f.getCanonicalPath());
                  list.add("<i>" + sd.toString() + "</i> written to <i>" + f.toString() + "</i> as a TIFF spectrum file.");
               } else {
                  exportSpectrumAsEMSA(sd, f.getCanonicalPath());
                  list.add("<i>" + sd.toString() + "</i> written to <i>" + f.toString() + "</i> as a EMSA/MSA 1.0 formatted file.");
               }
      ...
      if (list.size() > 0) {
         appendHTML(list.toString());
         if (errCx > 0)
            ErrorDialog.createErrorMessage(this, "File Save Errors",
                  Integer.toString(errCx) + (errCx == 1 ? " spectrum" : " spectra") + " not written successfully.", "<html>" + list.toString());
      }
   }
         ...
         if (nd.getOk()) {
            ...
            for (final ISpectrumData sd : getSelectedSpectra())
               for (int j = nd.getDuplicates() - 1; j >= 0; --j) {
                  final double liveTime = sd.getProperties().getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN);
                  if (!Double.isNaN(liveTime)) {
                     ...
                     try {
                        ...
                        list.addError("The spectrum <i>" + sd + "</i> was <b>not</b> sub-sampled: " + e.getLocalizedMessage());
                        list.setError("One or more spectra was not sub-sampled correctly");
                     }
                  } else {
                     list.addError("The spectrum <i>" + sd + "</i> was <b>not</b> sub-sampled.  The live time was unavailable.");
                     list.setError("One or more spectra was not sub-sampled correctly");
                  }
               }
            if (list.size() > 0)
    ...
    *           The file name
    * @param newExt
    *           The new extension starting with a '.' (ie. ".msa")
      ...
      if (choice == JFileChooser.APPROVE_OPTION) {
         ...
         for (final ISpectrumData spec : getSelectedSpectra())
            try {
               ...
               if (fn != null)
                  list.add("The spectrum <i>" + spec.toString() + "</i> was exported as CSV to <i>" + fn + "</i>");
               else {
   ...
   public void fileBatchExportEMSA() {
      ...
      if (choice == JFileChooser.APPROVE_OPTION) {
         ...
         for (final ISpectrumData spec : getSelectedSpectra())
            try {
               try {
                  ...
                  list.add("The spectrum <i>" + spec.toString() + "</i> was exported as EMSA to <i>" + fn + "</i>");
               } catch (final Exception ex) {
         ...
         if (list.size() > 0) {
            appendHTML(list.toString());
            ErrorDialog.createErrorMessage(MainFrame.this, "Batch export", "There was one or more errors batch exporting spectra.", list.toString());
         }
      }
   }
   ...
   public void processStripBackground() {
      ...
      for (final ISpectrumData sd : getSelectedSpectra()) {
         final ISpectrumData res = PeakStripping.Clayton1987.getStrippedSpectrum(SpectrumUtils.applyZeroPeakDiscriminator(sd));
   ...
   public void processFitBackground() {
      ...
      // final BremsstrahlungAnalytic ba = new
      // BremsstrahlungAnalytic.Castellano2004aBremsstrahlung();
      final HTMLList list = new HTMLList();
         ...
         try {
            ...
            if (comp != null) {
               ...
               if (rois.size() > 0) {
                  ...
                  bkg.getProperties().setTextProperty(SpectrumProperties.SpectrumDisplayName, "Brem[" + sd.toString() + ", manual rois]");
                  mDataManager.addSpectrum(bkg);
               } else {
                  final ISpectrumData bkg = ba.fitBackground2(det, sd, comp);
                  bkg.getProperties().setTextProperty(SpectrumProperties.SpectrumDisplayName, "Brem[" + sd.toString() + ", auto rois]");
                  mDataManager.addSpectrum(bkg);
               }
            }
         } catch (final EPQException ex) {
            list.addError("Failure fitting <i>" + TextUtilities.normalizeHTML(sd.toString()) + "</i>" + TextUtilities.normalizeHTML(ex.getMessage()));
   ...
   public void processSmooth() {
      ...
      for (final ISpectrumData sd : getSelectedSpectra()) {
         ...
         list.add(
               "<i>" + TextUtilities.normalizeHTML(sd.toString()) + "</i> smoothed to <i>" + TextUtilities.normalizeHTML(spec.toString()) + "</i>");
      }
      if (list.size() > 0)
         appendHTML(list.toString());
   }
   ...
   public void processPeakSearch() {
      ...
      for (final ISpectrumData sd : getSelectedSpectra()) {
         ...
         peaks.getProperties().setTextProperty(SpectrumProperties.SpectrumDisplayName, "Peaks[" + sd.toString() + "]");
         list.add("Peaks in <i>" + TextUtilities.normalizeHTML(sd.toString()) + "</i> recorded in <i>" + TextUtilities.normalizeHTML(peaks.toString())
               + "</i>");
   ...
   public void toolsEditMaterial() {
      ...
      if (specs.size() > 0) {
         ...
         for (final ISpectrumData sd : specs) {
            final Composition c = sd.getProperties().getCompositionWithDefault(SpectrumProperties.StandardComposition, null);
   ...
   /**
    * Adds a tab containing the component to the utility panel with the
    * specified title and toolTip.
    *
    * @param title
    * @param component
    * @param toolTip
