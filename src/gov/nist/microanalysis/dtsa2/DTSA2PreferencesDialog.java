...
import gov.nist.microanalysis.dtsa2.AppPreferences.Appearance;

class DTSA2PreferencesDialog extends PreferenceDialog {
   ...
   class BasicPreferences extends PreferencePanel {
      ...
      private final JComboBox<Appearance> jCombo_LaF = new JComboBox<>();
      private final JList<File> jList_JythonLibs = new JList<>();
      private final DefaultListModel<File> model_JythonLibs = new DefaultListModel<>();
      private final JButton jButton_AddLib = new JButton("Add");
      private final JButton jButton_RemoveLib = new JButton("Remove");
      ...
      private void initialize() {
         final FormLayout fl = new FormLayout("5dlu, right:pref, 5dlu, 150dlu, 3dlu, pref, 3dlu, pref",
               "pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, top:40dlu, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref");
         final PanelBuilder pb = new PanelBuilder(fl, this);
         final CellConstraints cc = new CellConstraints();
         int row = 1;
         pb.addSeparator("User Information", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Your &name", cc.xy(2, row + 2));
         pb.add(jTextField_UserName, cc.xyw(4, row + 2, 3));
         row = 5;
         pb.addSeparator("HTML Report Location", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Base report &path", cc.xy(2, row + 2));
         pb.add(jTextField_ReportPath, cc.xy(4, row + 2));
         jTextField_ReportPath.setEditable(false);
         pb.add(jButton_ReportPath, cc.xy(6, row + 2));
         row = 9;
         pb.addSeparator("Jython Library Paths", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Library Paths (restart required)", cc.xy(2, row + 2));
         pb.add(new JScrollPane(jList_JythonLibs), cc.xy(4, row + 2));
         model_JythonLibs.addAll(AppPreferences.getInstance().getJythonLibraryPaths());
         jList_JythonLibs.setModel(model_JythonLibs);
         {
            Panel btns = new Panel(new FormLayout("pref", "pref, 3dlu, pref"));
            btns.add(jButton_AddLib, cc.xy(1, 1));
            btns.add(jButton_RemoveLib, cc.xy(1, 3));
            pb.add(btns, cc.xy(6, row + 2));
         }
         row = 13;
         pb.addSeparator("Start-up script", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Python script", cc.xy(2, row + 2));
         pb.add(jTextField_Script, cc.xy(4, row + 2));
         jTextField_Script.setEditable(false);
         pb.add(jButton_ScriptPath, cc.xy(6, row + 2));
         row = 17;
         pb.addSeparator("Shut-down script", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Python script", cc.xy(2, row + 2));
         pb.add(jTextField_PostScript, cc.xy(4, row + 2));
         jTextField_PostScript.setEditable(false);
         pb.add(jButton_PostScriptPath, cc.xy(6, row + 2));
         row = 21;
         pb.addSeparator("EPQ Library JavaDoc Location", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("JavaDoc location", cc.xy(2, row + 2));
         pb.add(jTextField_EPQDoc, cc.xy(4, row + 2));
         jTextField_EPQDoc.setEditable(false);
         pb.add(jButton_EPQDocPath, cc.xy(6, row + 2));
         pb.add(jButton_EPQDocDefault, cc.xy(8, row + 2));
         row = 25;
         pb.addSeparator("Look-and-Feel", cc.xyw(1, row, fl.getColumnCount()));
         pb.addLabel("Appearance (requires restart)", cc.xy(2, row + 2));
         for (Appearance laf : Appearance.values())
            jCombo_LaF.addItem(laf);
         pb.add(jCombo_LaF, cc.xy(4, row + 2));

         jButton_AddLib.addActionListener(new ActionListener() {

            @Override
            public void actionPerformed(ActionEvent e) {
               final JFileChooser fc = new JFileChooser(AppPreferences.getInstance().getBaseReportPath());
               fc.setDialogTitle("Select the directory containing a Jython library");
               fc.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
               fc.setFileHidingEnabled(false);
               if (fc.showDialog(BasicPreferences.this, "Select") == JFileChooser.APPROVE_OPTION) {
                  model_JythonLibs.addElement(fc.getSelectedFile());
               }
            }
         });

         jButton_RemoveLib.addActionListener(new ActionListener() {
         ...
         jButton_EPQDocDefault.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
               jTextField_EPQDoc.setText(AppPreferences.EPQ_JAVA_DOC_DEFAULT);
            }
         });
         ...
         class ScriptActionListener implements ActionListener {
            ...
            public void actionPerformed(ActionEvent e) {
               ...
               final SimpleFileFilter sff = new SimpleFileFilter(new String[]{"py",}, "Python script");
         ...
         jTextField_ReportPath.setText(prefs.getBaseReportPath());
         jTextField_UserName.setText(prefs.getUserName());
         jTextField_Script.setText(prefs.getStartupScript().equals("") ? CLEAR : prefs.getStartupScript());
         jTextField_PostScript.setText(prefs.getShutdownScript().equals("") ? CLEAR : prefs.getShutdownScript());
         jTextField_EPQDoc.setText(prefs.getEPQJavaDoc());
      ...
      public void commit() {
         var prefs = AppPreferences.getInstance();
         prefs.setUserName(jTextField_UserName.getText());
         prefs.setStartupScript(jTextField_Script.getText().equals(CLEAR) ? "" : jTextField_Script.getText());
         prefs.setShutdownScript(jTextField_PostScript.getText().equals(CLEAR) ? "" : jTextField_PostScript.getText());
         prefs.setEPQJavaDoc(jTextField_EPQDoc.getText());
         prefs.setAppearance((Appearance) jCombo_LaF.getSelectedItem());
         prefs.setBaseReportPath(jTextField_ReportPath.getText());
         ArrayList<File> paths = new ArrayList<>();
         for (int i = 0; i < model_JythonLibs.size(); ++i)
            paths.add(model_JythonLibs.get(i));
         prefs.setJythonLibraryPaths(paths);
      }

   }

   class QuantPreferences extends PreferencePanel {
      ...
      private void initialize() throws Exception {
         ...
         jComboBox_ZAFAlgorithm.setSelectedItem(prefs.getCorrectionAlgorithm());
         ...
         jComboBox_MAC.setSelectedItem(prefs.getMACAlgorithm());
         ...
         jComboBox_BremAngular.setSelectedItem(prefs.getBremsstrahlungAngularDistribution());
         ...
         jComboBox_Ionization.setSelectedItem(prefs.getIonizationCrossSection());
         ...
         jCheckBox_VariableFF.setSelected(prefs.useVariableFF());
         add(jCheckBox_VariableFF, cc.xy(4, 11));
         jCheckBox_UseOFudge.setSelected(prefs.useOFudge());
      ...
      public void commit() {
         super.commit();
         var prefs = AppPreferences.getInstance();
         prefs.setCorrectionAlgorithm(jComboBox_ZAFAlgorithm.getSelectedItem().toString());
         prefs.setMACAlgorithm(jComboBox_MAC.getSelectedItem().toString());
         prefs.setIonizationCrossSection(jComboBox_Ionization.getSelectedItem().toString());
         prefs.setBremsstrahlungAngularDistribution(jComboBox_BremAngular.getSelectedItem().toString());
         prefs.setUseVariableFF(jCheckBox_VariableFF.isSelected());
         prefs.setUseOFudge(jCheckBox_UseOFudge.isSelected());
      }
   }

   private class InstrumentPanel extends PreferencePanel {
      ...
      private boolean checkDetectorGUID(EDSDetector det) {
         ...
         final Set<DetectorProperties> allDetectors = DTSA2.getSession().getAllDetectors();
            ...
            public void focusLost(FocusEvent e) {
               ...
               if (newName.length() > 0) {
                  ...
                  if ((name == null) || (!name.equals(newName))) {
                     ...
                     DTSA2.getSession().updateInstrument(mInstrument);
                  }
               }
            }
         });
         ...
         imprt.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
               try {
                  ...
                  final SimpleFileFilter xdet = new SimpleFileFilter(new String[]{"xdet"}, "XML detector Files (*.xdet)");
                  ...
                  if (fc.showOpenDialog(InstrumentPanel.this) == JFileChooser.APPROVE_OPTION) {
                     ...
                     try {
                        ...
                        while (!ok) {
                           final int res = JOptionPane.showConfirmDialog(InstrumentPanel.this,
                                 "It appears like this detector has already been imported?\nShould we continue to import this detector as a new detector?",
                                 "Importing a detector", JOptionPane.YES_NO_OPTION);
                           if (res == JOptionPane.YES_OPTION) {
                              ...
                              while (!ok) {
                                 ...
                                 ++iter;
                                 dp.getProperties().clear(new SpectrumProperties.PropertyId[]{SpectrumProperties.DetectorGUID});
   ...
   class AddInstrument extends PreferencePanel {
      ...
      private void load() {
         final Set<EDSDetector> xrds = DTSA2.getSession().getEDSDetectors(false);
         ...
         for (final ElectronProbe ep : DTSA2.getSession().getElectronProbes().keySet()) {
            InstrumentPanel ip = ei.get(ep);
            if (ip == null) {
               ip = new InstrumentPanel(getPreferenceDialog(), ep);
               addChildPanel(ip);
   ...
   abstract class DetectorPanel extends PreferencePanel {
       ...
       * @throws EPQException
       */
      abstract public EDSDetector getModified() throws EPQException;
   ...
   private class SiLiPanel extends DetectorPanel {
      ...
      SiLiPanel(PreferenceDialog pref, EDSDetector det, boolean newDet) throws EPQException {
      ...
      private void initialize() {
         ...
         jCheckBox_Enable.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
               ...
               DTSA2.getSession().setEnabled(dp, selected);
               assert DTSA2.getSession().isEnabled(dp) == selected;
            }
         ...
         imprt.addLabel(
               "<html>Often vendor's spectrum files will contain some of the data items in this panel. Use import to populate the panel with the vendor's values. Use care. Vendor's values are sometimes wrong.</html>",
               cc.xyw(3, 3, 3));
         ...
         final PanelBuilder crystal = new PanelBuilder(
               new FormLayout(xFmt, "pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 2dlu"));
         ...
         jButton_Export.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
               ...
               jfc.setMultiSelectionEnabled(false);
               jfc.setAcceptAllFileFilterUsed(true);
               final SimpleFileFilter sff = new SimpleFileFilter(new String[]{"xdet",}, "XML detector file (*.xdet)");
               jfc.addChoosableFileFilter(sff);
               jfc.setAcceptAllFileFilterUsed(true);
               jfc.setFileFilter(sff);
         ...
         // Update values
         jCheckBox_Enable.setSelected(isNewDetector() || DTSA2.getSession().isEnabled(mOriginal.getDetectorProperties()));
      ...
      public boolean isModified() {
         if (!jTextField_Name.getText().equals(mOriginal.getName()) || jTextField_GUID.getText().equals(AUTO_GENERATE)
               || !((String) jComboBox_Window.getSelectedItem()).equals(mOriginal.getWindow().getName()))
            return true;
      ...
      @Override
      public EDSDetector getModified() throws EPQException {
         ...
         if (isModified() || isNewDetector()) {
            try {
               ...
               if (isNewDetector()) {
                  ...
                  DTSA2.getSession().addDetector(dp);
                  DTSA2.getSession().addCalibration(dp, mod.getCalibration());
                  mNewDetector = false;
                  enableDetector(jCheckBox_Enable.isSelected());
               } else
                  // Update the previous detector to reflect the edited values
                  DTSA2.getSession().updateDetector(mOriginal.getDetectorProperties(), dp);
               mOriginal = mod;
            } catch (final EPQException e) {
               e.printStackTrace();
            }
   ...
   private class MicrocalPanel extends DetectorPanel {
      ...
      MicrocalPanel(PreferenceDialog pref, EDSDetector det, boolean newDet) throws EPQException {
         super(det, pref, "Edit the properties of this detector.");
         mNewDetector = newDet;
         mOriginal = det;
         try {
   ...
   public DTSA2PreferencesDialog(Frame owner) {
